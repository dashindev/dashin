...
--
-- Note: ids are plain INTEGER PRIMARY KEY (NOT AUTOINCREMENT) on purpose. The
-- seed wipes each table with DELETE then re-inserts, and without AUTOINCREMENT
-- the ids restart at 1 every reseed — so category/customer ids stay stable
-- (1..N) and the FK relationships (products.category_id, orders.customer_id)
-- keep matching the frontend's static lookup maps.
