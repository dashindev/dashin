...
Create a new command line window, enter the dashin **root directory** **_(dashin-tutorial)_**. **!NOT** ~~**_packages/dashin_**~~

```shell
# cd ../..
mkdir plugins/dashin-plugin-myteam-blog
```

_Folders starting with `dashin-plugin-` in the `plugins` directory will be automatically recognized as plugins._
...
Replace the content of `utils/initData.ts` **_(dashin-tutorial/plugins/dashin-plugin-myteam-blog/utils/initData.ts)_** with the following:
...
_If you see the screenshot page, it means that the plugin has been created and successfully activated, otherwise please check the steps above or try to restart dashin._

## [Sample code](https://github.com/dashindev/dashin-example-plugin-tutorial)
